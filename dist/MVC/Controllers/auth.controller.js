"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const mongoose_1 = __importDefault(require("mongoose"));
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const mailgun = require("mailgun-js");
const DOMAIN = "sandbox9ef16654558246898368760ecc18807f.mailgun.org";
const API_KEY = "51a767a7ce0baf3de8394e657cf1b7ef-1b8ced53-b7fdf769";
const mg = mailgun({ apiKey: API_KEY, domain: DOMAIN });
const bcrypt_1 = __importDefault(require("bcrypt"));
const saltRounds = 10;
require("../models/doctor.model");
const Doctor = mongoose_1.default.model("doctors");
require("../models/employee.model");
const Employee = mongoose_1.default.model("employees");
class AuthController {
    constructor() {
        this.login = (request, response, next) => {
            let User = null;
            if (request.params.userType == "doctor") {
                User = Doctor;
            }
            else if (request.params.userType == "employee") {
                //   User = Employee;
            }
            else {
                next(new Error("invalid user type!"));
            }
            User.findOne({ email: request.body.email }).then((data) => {
                if (data && bcrypt_1.default.compareSync(request.body.password, data.password)) {
                    let token = jsonwebtoken_1.default.sign({
                        id: data._id,
                        role: data.role,
                    }, "mysecret", { expiresIn: "1h" });
                    response.status(200).json({ token: token, role: data.role });
                }
                else {
                    let error = new Error("email or password incorrect");
                    error.status = 401;
                    next(error);
                }
            });
        };
        this.signUp = (request, response, next) => {
            console.log("signUp controller");
            let User = null;
            if (request.params.userType == "doctor") {
                User = Doctor;
            }
            else if (request.params.userType == "employee") {
                // User = Employee;
                //  delete request.body.clinics;
            }
            else {
                next(new Error("invalid user type!"));
            }
            let object = new User({
                _id: new mongoose_1.default.Types.ObjectId(),
                fullName: request.body.fullName,
                clinics: request.body.clinics,
                email: request.body.email,
                password: bcrypt_1.default.hashSync(request.body.password, saltRounds),
            });
            User
                .findOne({ fullName: request.body.fullName })
                .then((duplicateUsers) => {
                if (!duplicateUsers) {
                    return object.save().then((data) => {
                        response.status(201).json({ msg: "user created", data });
                    });
                }
                else {
                    next(new Error("user fullName already exist"));
                }
            })
                .catch((error) => next(error));
        };
        this.changePassword = (request, response, next) => {
            console.log("changePassword controller");
            let User = null;
            if (request.params.userType == "doctor") {
                User = Doctor;
            }
            else if (request.params.userType == "employee") {
                //   User = Employee;
            }
            else {
                next(new Error("invalid user type!"));
            }
            User
                .findOne({ _id: request.id }, { email: 1, password: 1 })
                .then((data) => {
                if (!data)
                    next(new Error("user not found"));
                if (data.email == request.body.email) {
                    if (bcrypt_1.default.compareSync(request.body.oldPassword, data.password)) {
                        if (bcrypt_1.default.compareSync(request.body.newPassword, data.password)) {
                            next(new Error("you entered the same password"));
                        }
                        data.password = bcrypt_1.default.hashSync(request.body.newPassword, saltRounds);
                        return data.save().then((data) => {
                            response.status(200).json({ msg: "user changed password", data });
                        });
                    }
                    else
                        next(new Error("old password incorrect!"));
                }
                else
                    next(new Error("incorrect email"));
            })
                .catch((error) => next(error));
        };
        this.forgotPassword = (request, response, next) => {
            let User = null;
            if (request.params.userType == "doctor") {
                User = Doctor;
            }
            else if (request.params.userType == "employee") {
                User = Employee;
            }
            else {
                next(new Error("invalid user type!"));
            }
            User
                .findOne({ email: request.body.email })
                .then((user) => {
                if (!user)
                    throw new Error("user is Not Exist!");
                let token = jsonwebtoken_1.default.sign({ _id: user._id, email: request.body.email }, "mySecret", {
                    expiresIn: "10m",
                });
                const data = {
                    from: "samaahamdy719@gmail.com",
                    to: request.body.email,
                    subject: "Password Reseting Link",
                    html: `
          <h2 style="color: red; font-size: 50px">Password Reseting</h2>
          <pre style="font-size: 30px;">
          Hello ${user.fullName},
          
          Please click on the given link to reset password..
        <a href="http://localhost:8080/authentication/resetPassword/${token}">Reset Password</a>
          If you didn't try reset yor password, ignore this email.
          </pre>
        `,
                };
                user.resetLink = token;
                mg.messages().send(data, function (error) {
                    if (error)
                        next(error);
                    response.status(200).json({
                        data: "Reset Password link has been sent to your Email, kindly check your mail and follow the instructions",
                    });
                });
            })
                .catch((error) => next(error));
        };
        this.resetPassword = (request, response, next) => {
            let token = request.params.token;
            let User = null;
            if (request.params.userType == "doctor") {
                User = Doctor;
            }
            else if (request.params.userType == "employee") {
                //   User = Employee;
            }
            else {
                next(new Error("invalid user type!"));
            }
            User
                .findOne({ resetLink: token })
                .then((user) => {
                if (!user)
                    next(new Error("user with this token doesn't exist!"));
                if (bcrypt_1.default.compareSync(request.body.newPassword, user.password)) {
                    next(new Error("you entered the same password"));
                }
                else {
                    user.password = bcrypt_1.default.hashSync(request.body.newPassword, saltRounds);
                    user.resetLink = "";
                    return user.save().then(() => {
                        response
                            .status(200)
                            .json({ msg: "Password Reseting done successfully" });
                    });
                }
            })
                .catch((error) => next(error));
        };
    }
}
exports.default = AuthController;
